    ...
    for (const segment of segments) {
        ...
        for (const item of current) {
            ...
            if (segment.type === 'property') {
                // Property access: .key
                if (segment.name && typeof item === 'object' && segment.name in item) {
                    next.push(item[segment.name]);
                }
                ...
                // Array index: [0], [1], etc.
                if (segment.index !== undefined && Array.isArray(item) && segment.index >= 0 && segment.index < item.length) {
 ...
 * ```
 */
export function extractByPaths(obj: any, paths: string[]): any[] {
