...
import { LlmMessage, ToolResult, ContextConfig } from "../types.js";
...
export function summarizeOldContext(messages: LlmMessage[]): string {
  ...
  // Count message types
  const userMsgCount = nonSystemMessages.filter(
    (m) => m.role === "user",
  ).length;
  const assistantMsgCount = nonSystemMessages.filter(
    (m) => m.role === "assistant",
  ).length;
  const toolMsgCount = nonSystemMessages.filter(
    (m) => m.role === "tool",
  ).length;

  summary.push(
    `Prior conversation: ${userMsgCount} user messages, ${assistantMsgCount} assistant responses, ${toolMsgCount} tool results.`,
  );
  ...
  if (userMessages.length > 0) {
    ...
    const lastUserMsg = userMessages[userMessages.length - 1].content.slice(
      0,
      100,
    );
...
export class ContextManager {
  constructor(
    private readonly config: ContextConfig = DEFAULT_CONTEXT_CONFIG,
  ) { }
